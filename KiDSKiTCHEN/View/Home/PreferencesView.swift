    ...
    var body: some View {
        List {
            ...
            if !prefs.excluded.isEmpty {
                Section("Ausgeschlossen (\(prefs.excluded.count))") {
                    ForEach(prefs.excluded.sorted(), id: \.self) { name in
                        Button(role: .destructive) {
                            prefs.toggleExcluded(name)
                        } label: {
                            Label(name, systemImage: "minus.circle.fill")
                        }
                    }
                }
            }
                    ...
                    ForEach(section.items) { ingredient in
                        Button {
                            ...
                            HStack {
                                ...
                                Spacer()
                                if prefs.excluded.contains(ingredient.name) {
